...
fun SignUpScreen(
    ...
    AuthPage {
        AuthTitle("Create your account", "Start sharing with more intention.")
        ...
        AuthTextField(uiState.confirmPassword, viewModel::updateConfirmPassword, "Confirm password", isPassword = true)
        ...
        ) { if (uiState.isLoading) CircularProgressIndicator(color = Color.White) else Text("Create account") }
        TextButton(onClick = onLogIn) { Text("Already have an account? Log in", color = Color.White) }
    }
}
