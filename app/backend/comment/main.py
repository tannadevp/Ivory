...
TEMPLATE_BANK = {
    "toxicity": [
        "I disagree with this, but I'd like to understand your point of view.",
        "I don't think that's right, and here's why.",
        "Let's talk about this differently.",
    ],
    "hate": [
        "I have a strong disagreement with this, but let's keep it respectful.",
        "I don't share that view, and I don't think generalizing about groups is fair.",
    ],
    "harassment": [
        "I'm frustrated with this, but I don't want to attack you personally.",
        "Can we discuss this without insults?",
    ],
    "abuse": [
        "I'm upset about this, but I don't want to be abusive about it.",
        "This bothers me a lot, but let's keep this civil.",
    ],
}

_template_embeddings = {
    label: _embedder.encode(templates, convert_to_tensor=True)
    for label, templates in TEMPLATE_BANK.items()
}


def suggest_alternative(text: str, per_label: dict) -> Optional[dict]:
    """Pick the most strongly flagged category, retrieve the closest-matching
    safe template by embedding similarity, then re-run OUR OWN classifier on
    the chosen template as a verifier before returning it."""
    flagged = {label: v["prob"] for label, v in per_label.items() if v["flagged"]}
    if not flagged:
        return None

    top_category = max(flagged, key=flagged.get)
    candidates = TEMPLATE_BANK[top_category]
    cand_embeddings = _template_embeddings[top_category]

    text_embedding = _embedder.encode(text, convert_to_tensor=True)
    scores = util.cos_sim(text_embedding, cand_embeddings)[0]
    best_idx = int(scores.argmax())
    chosen = candidates[best_idx]

    verification = predict(chosen, top_n=0)
    verified_safe = not verification["any_flagged"]
...
def suggest(req: SuggestRequest):
    if not req.text or not req.text.strip():
        raise HTTPException(status_code=400, detail="text must not be empty")
    result = predict(req.text, top_n=0)
    suggestion = suggest_alternative(req.text, result["per_label"])
    return {
        "original_analysis": {
